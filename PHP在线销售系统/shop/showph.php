...
<table width="800" height="438" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td width="590" valign="top"><table width="550" height="25" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        <td><img src="images/paihang.gif" width="250" height="25"></td>
      </tr>
    </table>
	  ...
	  <?php
	   $sql=mysql_query("select count(*) as total from shangpin where cishu>0  ",$conn);
	   $info=mysql_fetch_array($sql);
	   $total=$info["total"];
	   if($total==0)
	   {
	     echo "<div align='center'>本站还没有销售产品!</div>";
	   }
	   else
	   {
     ...
     <?php
		   ...
		   if ($total<=$pagesize){
			...
			if(($_GET["page"])==""){
             ...
             $sql1=mysql_query("select * from shangpin order by cishu desc limit ".($page-1)*$pagesize.",$pagesize ",$conn);
      ...
      <table width="550" height="25" border="0" align="center" cellpadding="0" cellspacing="0">
        ...
        <?php
		  ...
		  {
		  ?>
        <a href="showph.php?page=1" title="首页"><font face="webdings"> 9 </font></a> 
		<a href="showph.php?id=<?php echo $id;?>&page=<?php echo $page-1;?>" title="前一页"><font face="webdings"> 7 </font></a>
        <?php
		  }
		   if($pagecount<=4){
		    for($i=1;$i<=$pagecount;$i++){
		  ?>
        <a href="showph.php?page=<?php echo $i;?>"><?php echo $i;?></a>
        <?php
		     }
		   }else{
		   for($i=1;$i<=4;$i++){	 
        ...
        <a href="showph.php?page=<?php echo $i;?>"><?php echo $i;?></a>
        <?php }?>
        <a href="showph.php?page=<?php echo $page-1;?>" title="后一页"><font face="webdings"> 8 </font></a>
		 <a href="showph.php?id=<?php echo $id;?>&page=<?php echo $pagecount;?>" title="尾页"><font face="webdings"> : </font></a>
        <?php }?>
		  </div></td>
        </tr>
      </table>
